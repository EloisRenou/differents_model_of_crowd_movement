...
from tkinter import Canvas, Tk, Label,  TOP
...
def obstacle (x : float, y : float, dx : float, dy : float) -> tuple: #tuple(float,float)
    ...
    if x>17*taille+rayon and y<6.5*taille+rayon and y>5.5*taille-rayon:
        d=sqrt((16.5*taille-rayon-x-dx)**2+(7.5*taille+rayon-y-dy)**2)
        dx,dy=(16.5*taille-rayon-x-dx)/d, (7.5*taille+rayon -y-dy)/d
    ...
    if x>17.5*taille-rayon and y<12*taille+rayon and y>11*taille-rayon:
        d=sqrt((17.25*taille+rayon-x-dx)**2+(10*taille+rayon-y-dy)**2)
        dx,dy= (17.25*taille+rayon-x-dx)/d, (10*taille+rayon -y-dy)/d
    if x>16.5*taille+rayon and x<17.5*taille-rayon and y<11*taille+rayon and y>9*taille:
    ...
    if y<2*taille+rayon and x<15.5*taille+rayon and x>14.5*taille-rayon:
        d=sqrt((14*taille-rayon-x-dx)**2+(2.5*taille-rayon-y-dy)**2)
        dx,dy= (14*taille-rayon-x-dx)/d, (2.5*taille-rayon -y-dy)/d
    if x<15.5*taille+rayon and x>15*taille and y<4*taille+rayon and y>2*taille:
        dx=0
    if x>14.8*taille and x<18.5*taille-rayon and y<5.5*taille-rayon+4 and y>5.5*taille-rayon:
        dy=0
    if x>17*taille+rayon and x<17*taille+1.1*rayon and y>5.5*taille-rayon and y<6.5*taille+rayon:
        dx=0
    ...
    while len(personnes)<nb_personne:
        ...
        if compteur:
            if x>13*taille+rayon:
                if y<8.5*taille:
                    d=(sqrt((13*taille-x)**2+(7*taille - y)**2))
                    dx, dy = (13*taille - x)/d, (7*taille - y)/d
                else:
                    d=(sqrt((13*taille-x)**2+(11.5*taille - y)**2))
                    dx, dy = (13*taille - x)/d, (11.5*taille - y)/d
            else:
...
def mouvement(nb_personne : int, personnes : list) -> None: #list(float, float, float, float, float, int, int)
...
fen = Tk()
fen.title("TIPE Mouvement de foule : musée (Eloïs RENOU 30191)")
...
rec12=can.create_rectangle(11*taille,4.5*taille,13*taille,6*taille,fill='black')

#obstacle
rec13=can.create_rectangle(4.5*taille+5,10.5*taille+5,7.5*taille-5,11*taille-5,fill='black') #petit rectangle du bas secteur milieu
rec14=can.create_rectangle(7*taille+5,6*taille+5,9*taille-5,8*taille-5,fill='black') #carre du milieu
rec15=can.create_rectangle(14.5*taille+5,2*taille+5,15.5*taille-5,4*taille-5,fill='black') #rectangle du haut secteur haut
rec16=can.create_rectangle(15*taille+5,5.5*taille+5,17*taille-5,6.5*taille-5,fill='black')# rectangle du bas secteur haut
rec17=can.create_rectangle(15.75*taille+5,12*taille+5,19*taille-5,13*taille-5,fill='black') #barre horizontale de la croix
rec18=can.create_rectangle(16.5*taille+5,11*taille+5,17.5*taille-5,14*taille-5,fill='black') #barre verticale de la croix
